    ...
    <section className="py-16 px-5 md:px-10 bg-white">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="absolute left-0 md:left-1/2 transform md:-translate-x-1/2 h-full w-0.5 bg-brass hidden md:block"></div>
          
          {steps.map((step, index) => (
            <div key={index} className={`flex items-center mb-10 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
              {/* Circle on timeline */}
              <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 w-8 h-8 bg-brass rounded-full border-4 border-white z-10"></div>
              
              {/* Content */}
              <div className={`flex-1 ${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}>
                <div className="bg-light p-6 rounded-lg shadow-sm">
                  <div className="flex items-center mb-2">
                    <span className="inline-block w-6 h-6 bg-brass text-white rounded-full flex items-center justify-center text-sm font-bold mr-3">
                      {index + 1}
                    </span>
